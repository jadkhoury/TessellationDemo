...
void Interpolate_phong(Triangle target_T, vec3 uvw, float alpha, out Vertex vertex)
{
...
void PhongInterpolation(Triangle mesh_t, vec2 uv, int poly_type, float alpha, out Vertex vertex)
{
	float u = uv.x, v = uv.y, w = 1-u-v;
	vec3 uvw = vec3(w,v,u);
	uvw = uvw / (u+v+w);

        Interpolate_phong(mesh_t, uvw, alpha, vertex);
}
