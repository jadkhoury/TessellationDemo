...
void Interpolate_phong(Triangle target_T, vec3 uvw, float alpha, out vec4 result_p, out vec4 result_n)
{
	...
	// requires swapping for some reason
	vec3 tc1 = uvw;
	vec3 tc2 = tc1*tc1;
	...
	result_n = normalize(tc1[0] * target_T.vertex[0].n  + tc1[1] * target_T.vertex[1].n  + tc1[2] * target_T.vertex[2].n);
//    result_n.w = 0;
	...
	//phongPos = normalize(phongPos);
	alpha *= 0.5;
	phongPos = (1.0-alpha)*barPos + alpha*phongPos;

	result_p = vec4(phongPos, 1);

}

void PhongInterpolation(uvec4 key, vec2 uv, int prim_type, float alpha, out vec4 final_pos, out vec4 final_n)
{
	...
	Interpolate_phong(mesh_t, uvw, alpha, final_pos, final_n);
}
