...
public:
    struct Ticks {
        struct {double cpu, gpu;} compute;
        struct {double cpu, gpu;} batch;
        struct {double cpu, gpu;} render;
    } ticks;
    ...
    void Draw(float deltaT)
    {
        ...
        djgc_ticks(compute_clock_, &ticks.compute.cpu, &ticks.compute.gpu);
        ...
        glUseProgram(0);
        djgc_stop(batch_clock_);
        djgc_ticks(batch_clock_, &ticks.batch.cpu, &ticks.batch.gpu);
...
RENDER_PASS:
        ...
        glUseProgram(0);
        djgc_ticks(render_clock_, &ticks.render.cpu, &ticks.render.gpu);
    }
