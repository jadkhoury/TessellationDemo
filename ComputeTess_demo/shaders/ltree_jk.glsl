...
void lt_children_64(uvec2 nodeID, out uvec2 children[2])
{
    nodeID = lt_leftShift_64(nodeID, 1u);
    children[0] = uvec2(nodeID.x, nodeID.y | 0u);
    children[1] = uvec2(nodeID.x, nodeID.y | 1u);
}

uvec2 lt_parent_64(uvec2 nodeID) {
    return lt_rightShift_64(nodeID, 1u);
}

// --------------------------------- Level ---------------------------------- //

uint lt_level_64(uvec2 nodeID) {
    return lt_findMSB_64(nodeID);
}

// ------------------------------ Leaf & Root ------------------------------- //

bool lt_isLeaf_64(uvec2 nodeID) {
    return (lt_level_64(nodeID) == 63u);
}
...
bool lt_isZeroChild_64(uvec2 nodeID) {
    return ((nodeID.y & 1u) == 0u);
}

bool lt_isOneChild_64(uvec2 nodeID)
{
    return ((nodeID.y & 1u) == 1u);
}
...
void lt_getTriangleXform_64(uvec2 nodeID, out mat3x2 xform, out mat3x2 parent_xform)
{
    ...
    nodeID = lt_rightShift_64(nodeID, 1u);
    ...
    parent_xform = xf;
    xform = mul(parent_xform, jk_bitToMatrix(lsb & 1u));
}
...
{
    mat3x2 xform, pxform;
    lt_getTriangleXform_64(nodeID, xform, pxform);
...
vec4 lt_Leaf_to_MeshPosition(vec2 p, uvec4 key)
{
    uvec2 nodeID = key.xy;
    uint meshPolygonID = key.z;
    uint rootID = key.w & 1u;
    vec2 p2d = lt_Leaf_to_Tree_64(p, nodeID);
    return lt_Tree_to_MeshPosition(p2d, meshPolygonID, rootID);
}
